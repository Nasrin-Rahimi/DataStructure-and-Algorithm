...
function canTwoMoviesFillFlight(movieLengths, flightLength) {
    for(let i = 0; i < movieLengths; i++) {
        for(let  j = 0;  j < movieLengths; j++) {
            if(i !== j) {
                if (movieLengths[j] = flightLength - movieLengths[i]) {
                    return true;
                }
            }
        }
...
/**
To bring our runtime down we'll probably need to replace that inner loop (the one that 
looks for a matching secondMovieLength) with something faster.

We could sort the movieLengths first—then we could use binary search ↴ to find 
secondMovieLength in O(lgn) time instead of O(n) time. But sorting would cost O(nlg(n)), 
...
So we could throw all of our movieLengths into a set first, in O(n) time. Then we could loop 
through our possible firstMovieLengths and replace our inner loop with a simple check in 
our set. This'll give us O(n) runtime overall!

Of course, we need to add some logic to make sure we're not showing users the same movie twice...

But first, we can tighten this up a bit. Instead of two sequential loops, can we do it 
all in one loop? (Done carefully, this will give us protection from showing the same 
movie twice as well.)
 */
...
See if there's a matchingSecondMovieLength we've seen already (stored in our 
movieLengthsSeen set) that is equal to flightLength - firstMovieLength. 
...
What if we wanted to fill the flight length as nicely as possible with any number of 
movies (not just 2)?
...
Using hash-based data structures, like objects or sets, is so common in coding challenge 
solutions, it should always be your first thought. Always ask yourself, right from the 
