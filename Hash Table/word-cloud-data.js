...
cloud data in a map ↴ , where the keys are words and the values are the number of times 
the words occurred.

We'll use a JavaScript Map instead of an object because it's more explicit—we're 
mapping words to counts. And it'll be easier and cleaner when we want to iterate over our data.
...
should include one "Add" or "add" with a value of 22. Make reasonable (not necessarily 
perfect) decisions about cases like "After" and "Dana".

Assume the input will only contain words and standard punctuation.

You could make a reasonable argument to use regex in your solution. We won't, mainly 
because performance is difficult to measure and can get pretty bad.
*/

/**
Breakdown
We'll have to go through the entire input string, and we're returning a map with every 
unique word. In the worst case every word is different, so our runtime and space cost 
will both be at least O(n).
...
We could use the built-in split() function to separate our words, but if we just split on 
spaces we'd have to iterate over all the words before or after splitting to clean up the 
punctuation. And consider em dashes or ellipses, which aren't surrounded by spaces but 
nonetheless separate words. Instead, we'll make our own splitWords() function, which will 
...
/**
Careful—if you thought of building up the word character by character (using +=), you'd be 
doing a lot more work than you probably think. Every time we append a character to a 
string, Javascript makes a whole new string. If our input is one long word, then creating 
all these copies is O(n^2) time.

Instead, we keep track of the index where our word starts and its current length. Once we 
hit a space, we can use string slicing to extract the current word to append to the array. 
That keeps our split function at O(n) time.

Now we've solved the first part of the challenge, splitting the words. The next part is 
populating our map with unique words. What do we do with each word?

If the word is in the map, we'll increment its count. Otherwise, we'll add it to the map 
with a count of 1.
 */
...
Take some time to think of possible approaches. What are some other sentences you might 
run into. What are all your options?
...
- Make a word uppercase in our map if it is ever uppercase in the original string in a 
...
this will have reasonable accuracy in very long strings where words are more likely to be 
included multiple times, but words that only ever occur as the first word in a sentence will 
always be included as uppercase.
2- Make a word uppercase in our map if it is ever uppercase in the original string: this 
...
3- Make a word uppercase in our map if it is ever uppercase in the original string in a 
...
lowercase.
4- Use an API or other tool that identifies proper nouns: this has a lot of potential to 
give us a high level of accuracy, but we'll give up control over decisions, we'll be relying 
on code we didn't write, and our practical runtime may be significantly increased.
...
You can choose whichever of the options you'd like, or another option you thought of. 
For this breakdown, we're going to choose option (1).
...
1- Check for words that are in the map in both cases when we're done populating the map. 
If we add "Vanilla" three times and "vanilla" eight times, we'll combine them into one 
"vanilla" at the end with a value 1111.
2- Avoid ever having a word in our map that's both uppercase and lowercase. As we add 
"Vanilla"s and "vanilla"s, we'd always only ever have one version in our map.
We'll choose the second approach since it will save us a walk through our map. How should 
we start?

If the word we're adding is already in the map in its current case, let's increment its 
count. What if it's not in the map?

There are three possibilities:

1- A lowercase version is in the map (in which case we know our input word is uppercase, 
because if it is lowercase and already in the map it would have passed our first check 
and we'd have just incremented its count)
...
This is a little more complicated. We need to remove the uppercase version from our map 
...
/**
Now we have all our pieces! We can split words, add them to a map, and track the number 
...
string once and then every word in our array once. That's a runtime of O(n), which is the 
best we can achieve for this challenge (we have to look at the entire input string). 
The space we're using includes an array for each word and a map for every unique word. 
Our worst case is that every word is different, so our space cost is also O(n), which 
is also the best we can achieve for this challenge (we have to return a map of words).

But we can still make some optimizations!

How can we make our space cost even smaller?

We're storing all our split words in a separate array. That at least doubles the memory 
we use! How can we eliminate the need for that array?

Right now, we store each word in our array as we split them. Instead, let's just 
immediately populate each word in our map!
 */
