...
/**
 * 
The problem is like the longest-substring-with-at-most-two-distinct-characters
...
let totalFruit = function(fruits) {
    ...
    let left  = 0, right = 0, maxFruits = 2;
    let fruitsMap = new Map();

    while(right < len) {
        fruitsMap.set(fruits[right], right);

        if(fruitsMap.size == 0) {
            let smallestIndex = Math.min(...fruitsMap.values());
            fruitsMap.delete(fruits[smallestIndex]);
